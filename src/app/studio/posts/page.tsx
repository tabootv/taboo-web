...
import { useFilePreview, useMultiFilePreview } from '@/hooks/use-file-preview';
...
export default function CreatePostPage() {
  ...
  const imagePreview = useMultiFilePreview(4);
  const audioPreview = useFilePreview();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!caption.trim() && imagePreview.files.length === 0 && !audioPreview.file) {
    ...
    try {
      const response = await studio.createPost({
        body: caption,
        image: imagePreview.files[0] || null,
      });
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Card>
          <CardContent className="p-6">
            ...
            {imagePreview.previewUrls.length > 0 && (
              <div
                className={`mt-4 grid gap-2 ${
                  imagePreview.previewUrls.length === 1 ? 'grid-cols-1' : 'grid-cols-2'
                }`}
              >
                {imagePreview.previewUrls.map((preview, index) => (
                  <div
                    key={index}
                    className={`relative rounded-xl overflow-hidden ${
                      imagePreview.previewUrls.length === 1
                        ? 'aspect-video'
                        : imagePreview.previewUrls.length === 3 && index === 0
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={() => imagePreview.removeFile(index)}
              ...
              <div className="mt-4 p-4 rounded-xl bg-white/5 border border-white/10">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-white">{audioPreview.file?.name}</span>
                  </div>
                  <button
                    type="button"
                    onClick={removeAudio}
                ...
                <audio src={audioPreview.previewUrl} controls className="w-full" />
              </div>
            )}
            ...
            <div className="mt-6 pt-4 border-t border-white/10 flex items-center justify-between">
              <div className="flex gap-2">
                <label
                  className={`p-3 rounded-xl hover:bg-white/10 transition-colors cursor-pointer ${
                    imagePreview.files.length >= 4 ? 'opacity-50 cursor-not-allowed' : ''
                  }`}
                >
                  ...
                  <input
                    ...
                    disabled={imagePreview.files.length >= 4}
                    multiple
                ...
                <label
                  className={`p-3 rounded-xl hover:bg-white/10 transition-colors cursor-pointer ${
                    audioPreview.file ? 'opacity-50 cursor-not-allowed' : ''
                  }`}
                >
              ...
              <div className="flex items-center gap-2 text-xs text-white/40">
                {imagePreview.files.length > 0 && <span>{imagePreview.files.length}/4 images</span>}
                {audioPreview.file && <span>1 audio</span>}
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="flex justify-end gap-4">
          ...
          <Button
            type="submit"
            disabled={
              (!caption.trim() && imagePreview.files.length === 0 && !audioPreview.file) ||
              isPosting
            }
